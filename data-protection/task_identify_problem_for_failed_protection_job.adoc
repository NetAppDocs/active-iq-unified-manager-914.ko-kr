...
+
이 작업에는 가상 서버의 `cluster2_src_svm` 소스 볼륨과 이름이 인 가상 서버의 `cluster3_dst_svm` 대상 볼륨 `managed_svc2_vol3` 간의 보호 관계가 관련되었습니다. `cluster2_src_vol2`

* 소스 볼륨에서 `cluster2_src_svm:/cluster2_src_vol2` 에 대한 스냅샷 복사 작업이 실패했습니다 `0426cluster2_src_vol2snap`.
...
+
볼륨/상태 세부 정보 페이지가 `cluster2_src_vol2` 표시되고 보호 탭의 내용이 표시됩니다.
...
. 용량 * 탭을 클릭합니다.
+
볼륨에 대한 용량 `cluster2_src_vol2` 정보가 표시됩니다.
...
. Summary * (요약 *) 영역에서 이벤트의 원인 필드를 읽습니다 `The full threshold set at 90% is breached. 45.38 MB (95.54%) of 47.50 MB is used`.
...
+
.. 관련 장치 * 창에서 상위 집계를 `cluster2_src_aggr1` 확인합니다.
+
[TIP]
====
...
. System Manager로 이동하거나 ONTAP CLI에 액세스하여 옵션을 활성화합니다 `volume autogrow`.
+
[TIP]
====
