...
* 루트 파티션에 50GB가 할당됩니다
* 디렉토리에 할당된 100GB의 사용 가능한 디스크 `/opt/netapp/data` 공간으로, LVM 드라이브 또는 타겟 시스템에 연결된 별도의 로컬 디스크에 마운트됩니다


[NOTE]
====
별도로 마운트되어 있는 디렉토리와 디렉토리인 `/var/log` 경우 `/opt` 에 `/opt` 15GB `/var/log`의 여유 공간이 있는지 확인합니다.  `/tmp`디렉토리에는 최소 10GB의 여유 공간이 있어야 합니다.
