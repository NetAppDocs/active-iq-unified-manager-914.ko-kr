...
* Linux 시스템에서 SELinux(Security-Enhanced Linux)를 비활성화해야 합니다. 명령을 사용하여 SELinux 런타임 모드를 ""Permissive""로 변경합니다 `setenforce 0`.
...
이 작업이 완료되면 소프트웨어가 제거됩니다. 제거 후 디렉토리의 `/opt/netapp/data/BACKUP` 모든 데이터가 `/opt/netapp/data` 폴더로 이동됩니다. Unified Manager를 제거하면 Java 및 MySQL 패키지도 제거됩니다. 단, 시스템의 다른 애플리케이션에서 패키지가 필요하고 사용되지 않습니다. 그러나 MySQL 데이터는 삭제되지 않습니다.
