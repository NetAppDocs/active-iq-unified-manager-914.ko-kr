...
* NetApp Support 사이트에서 대상 시스템으로 Unified Manager 파일을 다운로드해야 `.zip` 합니다.
* 다운로드한 파일의 무결성을 확인해야 `.zip` 합니다.
...
+
종속성 오류가 발생하면 구성 요소를 제거하는 옵션을 추가해야 `--nodeps` 합니다.

. 설치 파일을 다운로드한 디렉토리로 `.zip` 이동하여 Unified Manager 번들을 확장합니다.
+
`unzip ActiveIQUnifiedManager-_<version>_.zip`

+
Unified Manager에 필요한 `.rpm` 모듈의 압축이 타겟 디렉토리에 풀립니다.
...
+
.. Unified Manager를 설치하는 시스템에서 사용 가능하고 사용할 수 없는 패키지 목록을 봅니다. +
...
[NOTE]
====
플러그인 ""yum-plugin-downloadonly""가 Red Hat Enterprise Linux 시스템에서 항상 활성화되어 있는 것은 아니므로 패키지를 설치하지 않고 다운로드할 수 있는 기능을 활성화해야 할 수 있습니다.
...
. 루트 사용자 또는 를 사용하여 `sudo` 다음 명령을 실행하여 소프트웨어를 설치합니다.
+
...
+
GPG NOKEY 오류로 인해 설치에 실패하면 를 사용하여 `rpm --import` URL에서 키를 가져옵니다.
...
[NOTE]
====
다른 명령(예:)을 사용하여 설치를 시도하지 `rpm -ivh` 마십시오. Red Hat Enterprise Linux 또는 CentOS 시스템에 Unified Manager를 성공적으로 설치하려면 모든 Unified Manager 파일과 관련 파일이 특정 순서로 설치되고 명령을 통해 자동으로 강제 적용되는 특정 디렉토리 구조에 설치되어야 합니다. `yum install netapp-um<version>.x86_64.rpm`
...
웹 UI에 액세스하여 umadmin 사용자의 기본 암호를 변경하고 에 설명된 대로 Unified Manager의 초기 설정을 수행합니다.link:../config/concept_configure_unified_manager.html["Active IQ Unified Manager 구성"] umadmin 사용자의 기본 암호를 변경해야 합니다.
