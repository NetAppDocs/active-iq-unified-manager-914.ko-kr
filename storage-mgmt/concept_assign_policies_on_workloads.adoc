...
권장 사항은 내부적으로 시스템 분석에 의해 결정되며, IOPS 및 기타 매개 변수가 사용 가능한 모든 PSL의 정의와 일치하지 않는 워크로드의 경우 건너뜁니다. 및 학습 상태가 있는 스토리지 작업 부하도 `Waiting for I/O` 제외됩니다.
...
====
기술 자료(KB) 문서도 참조하십시오 https://kb.netapp.com/Advice_and_Troubleshooting/Data_Infrastructure_Management/Active_IQ_Unified_Manager/Performance_Service_Level'_is_not_adaptive_to_a_highly_variable_workload["ActiveIQ Unified Manager '시스템 권장 성능 서비스 레벨 할당'은 워크로드가 매우 가변적인 워크로드에 대해 적응하지 못합니다"]
