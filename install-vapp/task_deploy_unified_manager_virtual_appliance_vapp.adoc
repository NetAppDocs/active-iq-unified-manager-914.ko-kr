...
Unified Manager 9.8부터 VMware Tools가 Open VM Tools로 대체되었습니다. `_open-vm-tools_` 는 Unified Manager 설치 패키지에 포함되어 있으므로 설치 프로세스의 일부로 VMware Tools를 설치할 필요가 없습니다. `_open-vm-tools_`
...
+
** Publisher 섹션의 세부 정보를 확인합니다. Entrust Code Signing-OVCS2 (Trusted certificate) * 메시지는 다운로드한 파일의 무결성을 `OVA` 확인합니다. + * Entrust Code Signing - OVCS2 (Invalid certificate) * 메시지가 표시되면 VMware vCenter Server를 7.0U3E 이상 버전으로 업그레이드합니다.
