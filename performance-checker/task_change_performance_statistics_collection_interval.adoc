...
성능 통계 수집이 정시에 완료되지 않는 문제는 배너 메시지 또는 `Data collection is taking too long on cluster <cluster_name>` 로 `Unable to consistently collect from cluster <cluster_name>` 표시됩니다.
