...
(`.pdf`보고서 * 버튼을 사용하여 이 페이지에서 데이터를 쉼표로 구분된 값(`.csv`) 파일, Microsoft Excel 파일(`.xlsx`) 또는) 문서로 내보낸 다음 내보낸 데이터를 사용하여 보고서를 작성할 수 있습니다. 또한 페이지를 사용자 정의한 다음 * Scheduled Reports * 버튼을 사용하여 정기적으로 보고서를 만들고 이메일로 전송할 수 있도록 예약할 수 있습니다.
...
특정 객체의 지연 시간 및 처리량을 분석하려면 More Options 버튼을 image:icon_kabob.gif[""] 클릭한 다음 * Analyze Workload * 를 클릭하면 Workload Analysis 페이지에서 성능 및 용량 차트를 확인할 수 있습니다. System Manager에 대한 유효한 자격 증명이 있는 경우 System Manager에서 세부 정보를 볼 수 있습니다.
...
특정 개체의 지연 시간 및 처리량을 분석하려면 자세히 아이콘을 클릭한 image:icon_kabob.gif[""]다음 * 워크로드 분석 * 을 클릭하면 * 워크로드 분석 * 페이지에서 성능 및 용량 차트를 확인할 수 있습니다.
...
* Associated Objects: QoS 정책 그룹에 할당된 워크로드 수입니다. QoS 정책 그룹 이름 옆에 있는 확장 단추(image:../media/chevron_down.gif[""])를 클릭하여 정책 그룹에 대한 자세한 정보를 볼 수 있습니다.
...
자세한 내용은 아래의 항목을 link:..//performance-checker/concept_manage_performance_using_qos_policy_group_information.html["QoS 정책 그룹 정보를 사용하여 성능 관리"]참조하십시오.
