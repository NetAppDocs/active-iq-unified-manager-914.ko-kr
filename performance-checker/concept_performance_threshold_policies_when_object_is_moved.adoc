...
명령을 사용하여 애그리게이트를 한 노드에서 다른 노드로 이동할 경우 `aggregate relocation start` 단일 및 조합 임계값 정책이 모든 개체에 유지되고 임계값 정책의 노드 부분이 새 노드에 적용됩니다.
...
자세한 내용은 을 link:../storage-mgmt/concept_volume_behavior_during_switchover_and_switchback.html["스위치오버 및 스위치백 중 볼륨 동작"]참조하십시오.
