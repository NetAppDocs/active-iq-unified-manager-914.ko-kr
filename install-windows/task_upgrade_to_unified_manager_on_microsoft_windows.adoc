...
[role="lead"]
Windows 플랫폼에서 설치 파일을 다운로드하여 실행하면 Unified Manager 9.12 또는 9.13을 9.14로 업그레이드할 수 있습니다.
...
+
을 link:reference_windows_software_and_installation_requirements.html["Windows 소프트웨어 및 설치 요구 사항"]참조하십시오.
...
====
* MySQL Community Edition은 Unified Manager 업그레이드 중에 자동으로 업그레이드됩니다. 시스템에 설치된 MySQL 버전이 8.0.34 이전인 경우 Unified Manager 업그레이드 프로세스에서 MySQL을 8.0.34로 자동 업그레이드합니다. MySQL 이전 버전의 독립 실행형 업그레이드를 8.0.34로 실행하면 안 됩니다.
* Windows 관리자 권한이 있어야 합니다. 사용자 이름이 ""!"로 시작하지 않도록!`". Installation of Unified Manager might fail if the user name of user running the installation begins with "` 하십시오.
...
[NOTE]
====
Unified Manager의 자동 업그레이드를 수행하려면 다음 명령을 실행합니다.
